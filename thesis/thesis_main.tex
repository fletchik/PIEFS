...
Предложенный метод (EFDO) оценивается на пяти наборах данных: Two Moons, Circles, HTRU2, MNIST и CIFAR-10 (с предобученными эмбеддингами ResNet-18).
...
We propose an alternative spectral framework (EFDO) based on eigenfunctions of a modified Dirichlet energy functional.
...
Глава~\ref{ch:method} описывает предложенный метод EFDO.
...
Метод EFDO (Eigenfunction-based Dirichlet-regularized Operator) строит $K$ нейросетевых аппроксимаций $\phi_1, \ldots, \phi_K$ путём последовательной минимизации обобщённой энергии Дирихле с сопутствующим обучением линейного классификатора.
...
\subsection{Тождественная матрица (EFDO-off)}
$\mathbf{A}(\mathbf{x}) = \mathbf{I}$ — базовый вариант без обучаемой метрики.

\subsection{Диагональная метрика (EFDO-diag)}
...
\subsection{Trotter-параметризация (EFDO-trotter)}
\begin{equation}
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{Датасет} & \textbf{RF} & \textbf{LR} & \textbf{NeuralEF} & \textbf{EFDO-off} & \textbf{EFDO-diag} & \textbf{EFDO-trotter} \\
    \midrule
...
\section{Почему EFDO-off лучше EFDO-diag/trotter?}
...
В работе предложен и исследован метод EFDO — обучаемые спектральные признаки на основе модифицированного функционала энергии Дирихле.
...
\textbf{Практическая значимость:} после завершения обучения EFDO выполняет один прямой проход для получения $K$ признаков, что делает метод пригодным для задач с большим числом тестовых примеров.
...
Исходный код доступен в репозитории:\\ \url{https://github.com/[anonymized]/efdo}
